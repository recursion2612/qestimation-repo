...
double prior_dist(double delta_in)
{
...
int main(int argc, char* argv[])
{
	...
	ofstream myFile("data_dm_linear.txt");
	for(int ii=0; ii<steps; ii++)
	{
		myFile<<dt_1*double(ii)<<"\t"<<avg_x(ii)<<"\t"<<avg_y(ii)<<'\t'<<avg_z(ii)<<'\t'<<prob_dist(0,ii)<<'\t'<<prob_dist(1,ii)<<'\t'<<tr_data(ii)<<'\t'<<countsss_arr(ii)<<endl;
	}
